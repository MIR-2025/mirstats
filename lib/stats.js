...
// Drop "phantom" sources caused by an upstream first-byte drop: a name that is
// exactly one leading character shorter than — and a tail of — a far higher-volume
// source (e.g. `afco` vs `nafco`). Tight criteria so real low-volume sources
// (helpdesk, mir, …) are never affected.
function dropTruncations(list) {
  return list.filter((s) => !list.some((t) =>
    t.key.length === s.key.length + 1 &&
    t.key.endsWith(s.key) &&
    t.count > s.count * 4));
}
...
function makeBucket() {
  // realErrors = 5xx + 4xx on normal routes (the number that should alarm you);
  // noiseErrors = 4xx on attack/probe paths (scanner 404 floods — decorative).
  const counts = { lines: 0, requests: 0, attacks: 0, bots: 0, assets: 0, alerts: 0, realErrors: 0, noiseErrors: 0 };
  const byStatus = { '1xx': 0, '2xx': 0, '3xx': 0, '4xx': 0, '5xx': 0, other: 0 };
  const byMethod = new Map();
  const byPath = new Map();
  const byIp = new Map();
  const byCountry = new Map();
  const brokenRoutes = new Map(); // "source\tpath" -> count of 404/5xx on a known-good route
  ...
  function record(rec) {
    ...
    if (rec.statusClass === '5xx') counts.realErrors++;
    else if (rec.statusClass === '4xx') {
      // For a mapped site, a 4xx is real only if it hits a known route; otherwise
      // the path never existed → noise. Unmapped sites fall back to the attack-path
      // heuristic (we can't tell route from typo without an index).
      const noise = rec.indexedSource ? !rec.knownRoute : rec.isAttack;
      if (noise) counts.noiseErrors++; else counts.realErrors++;
    }
    bump(byMethod, rec.method);
    if (rec.country) bump(byCountry, rec.country);
    if (rec.brokenRoute) bump(brokenRoutes, `${rec.source}\t${rec.path}`);
  ...
  function snapshot() {
    ...
    return {
      ...
      realErrorRate: +((counts.realErrors / reqs) * 100).toFixed(1),
      noiseRate: +((counts.noiseErrors / reqs) * 100).toFixed(1),
      ...
      topCountries: topEntries(byCountry, 12),
      brokenRoutes: [...brokenRoutes.entries()].sort((a, b) => b[1] - a[1]).slice(0, 10)
        .map(([k, c]) => { const i = k.indexOf('\t'); return { source: k.slice(0, i), path: k.slice(i + 1), count: c }; }),
  ...
  function restore(s) {
    ...
    // Backfill the real/noise error split for state saved before these counters
    // existed: scanner attacks are ~all 4xx, so count them as the noise 4xx.
    if (!counts.realErrors && !counts.noiseErrors && (byStatus['4xx'] || byStatus['5xx'])) {
      const noise4 = Math.min(counts.attacks || 0, byStatus['4xx'] || 0);
      counts.noiseErrors = noise4;
      counts.realErrors = (byStatus['5xx'] || 0) + ((byStatus['4xx'] || 0) - noise4);
    }
    for (const [k, v] of s.byMethod || []) byMethod.set(k, v);
    for (const [k, v] of s.byPath || []) byPath.set(k, v);
    for (const [k, v] of s.byIp || []) byIp.set(k, v);
    for (const [k, v] of s.byCountry || []) byCountry.set(k, v);
    for (const [k, v] of s.brokenRoutes || []) brokenRoutes.set(k, v);
...
export function createStats() {
  ...
  function snapshot(source) {
    ...
    snap.bySource = dropTruncations(topEntries(bySource, 60)); // all sources, minus upstream truncations
