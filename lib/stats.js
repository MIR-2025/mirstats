...
export function createStats() {
  ...
  function snapshot(source) {
    ...
    snap.bySource = topEntries(bySource, 20);
    snap.source = scoped ? source : null;
    return snap;
  }

  // Global cumulative state for cross-restart persistence (per-source buckets
  // are not persisted — they rebuild from the live feed).
  function serialize() {
    return { startedAt, bySource: [...bySource], global: global.serialize() };
  }
  function restore(s) {
    if (!s) return;
