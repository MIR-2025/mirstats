...
import { isProxyIp } from 'mir-sentinel';
...
function freshState(total = 0, lastReportAt = 0) {
  return { hits: 0, total, lastReportAt, paths: new Set(), lines: [], burst: [], burstSrc: null, busy: false, reason: 'fingerprint' };
}
...
// Restore the recent-reports ring from the audit log so the dashboard panel
// survives restarts (newest first, capped at RECENT).
function loadRecent() {
  try {
    const lines = readFileSync(AUDIT_FILE, 'utf8').trim().split('\n').filter(Boolean);
    return lines.slice(-RECENT).map((l) => { try { return JSON.parse(l); } catch { return null; } }).filter(Boolean).reverse();
  } catch {
    return [];
  }
}
...
export function createReporter(io) {
  ...
  const recent = loadRecent(); // recent report events, restored from the audit log
  ...
  function onRequest(rec) {
    ...
    if (!st) { st = freshState(); state.set(ip, st); }
    const now = Date.now();
    // Burst is per-source: a source switch resets the count, so a reported burst
    // is always N hits to ONE site (attackers hit one site at a time).
    if (st.burstSrc !== rec.source) { st.burst = []; st.burstSrc = rec.source; }
    st.burst.push(now);
