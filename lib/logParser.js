...
// Opt-in: on sites that serve no PHP, any .php request is a probe. Default off so
// real PHP sites don't false-positive. Enable with FLAG_PHP_PROBES=1.
const FLAG_PHP_PROBES = /^(1|true|yes|on)$/i.test(process.env.FLAG_PHP_PROBES || '');
...
function normalizePath(target) {
  ...
  // Strip a leading host (e.g. "example.com/...", "api.example.com/...").
  if (!t.startsWith('/')) {
...
export function parseLine(raw) {
  ...
  if (rec.path) {
    rec.isAttack = ATTACK_RE.test(rec.path) || (FLAG_PHP_PROBES && PHP_RE.test(rec.path));
    rec.isAsset = ASSET_RE.test(rec.path);
  }
  rec.isBot = BOT_RE.test(raw);
  // CDN / reverse-proxy edge IPs (e.g. Cloudflare) aren't the real client, so we
  // never blame or report them as attackers — see netblocks.js.
  rec.isProxy = isProxyIp(rec.ip);
