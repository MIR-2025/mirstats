// Tolerant parser for the heterogeneous MIR log stream.
//
// Lines arrive over the upstream `log:line` channel in many per-site formats
// (nafco/curio/mircapture/mirassertions/mir-org/.../apache-combined/etc), each
// optionally prefixed with `[source] ` by mir.org's externalLogTail. Rather than
// match every format exactly, we extract the few fields that matter for stats
// generically: source, method, path, status, ip — plus a handful of class flags.
...
export function parseLine(raw) {
  ...
  // Optional [source] prefix (alnum/-/_), added by externalLogTail. A bracketed
