...
async function ensureIndexes(database) {
  ...
  // Per-request event log for IP search: anchored-prefix search on `ip` + date
  // range, auto-pruned after ~1 year via TTL on `t`.
  await database.collection('events').createIndex({ ip: 1, t: -1 });
  await database.collection('events').createIndex({ t: 1 }, { expireAfterSeconds: 366 * 24 * 60 * 60 });
}
