...
export function createInfra(io) {
  ...
  function addHost(entry) {
    ...
    hosts.push(h);
    persist();
    pollHost(h).then(() => io.to('stats').emit('infra', latest())).catch(() => {});
    return configList();
  }
  function removeHost(label) {
    const i = hosts.findIndex((h) => h.label === label);
    if (i >= 0) { hosts.splice(i, 1); state.delete(label); prev.delete(label); persist(); io.to('stats').emit('infra', latest()); }
    return configList();
  }
  ...
  timer.unref?.();
  return { latest, configList, addHost, removeHost, stop: () => clearInterval(timer) };
}
