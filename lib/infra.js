...
const POLL_MS = Math.max(5000, +(process.env.INFRA_POLL_MS || 10000));
...
function parseLines(text) {
  const r = { cpu: null, mem: null, load: null, net: null, disk: null };
...
export function createInfra(io) {
  ...
  hosts = (Array.isArray(hosts) ? hosts : [])
    .map((h) => {
      if (typeof h === 'string') { const ssh = h.trim(); return ssh ? { label: labelFor(ssh), ssh } : null; }
      if (h && h.ssh) return { ...h, label: h.label || labelFor(String(h.ssh)) };
      return null;
    })
    .filter(Boolean);
  if (!hosts.length) {
    console.log(`[infra] no hosts in ${HOSTS_FILE} — infrastructure monitoring disabled`);
    return { latest: () => [], stop: () => {} };
  }
  ...
  async function pollAll() {
    ...
    io.to('stats').emit('infra', latest());
  }

  function latest() { return hosts.map((h) => state.get(h.label) || { label: h.label, host: h.label, offline: true, hist: [] }); }

  console.log(`[infra] monitoring ${hosts.length} host(s) every ${POLL_MS / 1000}s: ${hosts.map((h) => h.label).join(', ')}`);
  pollAll();
  const timer = setInterval(pollAll, POLL_MS);
  timer.unref?.();
  return { latest, stop: () => clearInterval(timer) };
}
