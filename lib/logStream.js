...
export function startLogStream(io) {
  ...
  // Periodic snapshot to all connected dashboards.
  const timer = setInterval(() => {
    io.to('stats').emit('stats', { upstream, ...stats.snapshot() });
  }, SNAPSHOT_MS);
  timer.unref?.();

  // Push a full snapshot immediately when a dashboard connects/joins.
  io.on('connection', (socket) => {
    socket.on('join', (room) => {
      if (room === 'stats') {
        socket.emit('stats', { upstream, ...stats.snapshot() });
      }
    });
  });
