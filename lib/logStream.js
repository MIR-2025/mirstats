...
export function startLogStream(io, { routes } = {}) {
  ...
  upstreamSocket.on('log:line', (line) => {
    ...
    // A known-good route returning 404/5xx is a *real* broken route (vs a 404 on
    // a path that never existed — scanner noise).
    if (rec.isRequest && rec.path && (rec.status === 404 || rec.statusClass === '5xx') && isKnownRoute(rec.source, rec.path)) rec.brokenRoute = true;
    stats.record(rec);
  ...
  const snapFor = (source) => ({
    ...
    rpmCur: history.current(), // current bucket, for live bar updates
    histBounds: history.bounds(), // earliest/latest minute available
    rpmBucket: history.bucket, // minutes per chart bar
    hits5: history.recentHits(5), // total hits over the last 5 minutes
  });
