...
export function createHistory() {
  ...
  // The in-progress current bucket (for live incremental bar updates).
  function current() {
    const b = BUCKET_MIN;
    const start = Math.floor(curMin / b) * b;
    const agg = zero();
    for (let m = start; m <= curMin; m++) { const x = hist.get(m); if (x) addInto(agg, x); }
