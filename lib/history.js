...
const BUCKET_MIN = Math.max(1, Math.floor(+(process.env.RPM_BUCKET_MIN || 1))); // minutes per chart bar
...
function addInto(agg, x) {
  agg['2xx'] += x['2xx']; agg['3xx'] += x['3xx']; agg['4xx'] += x['4xx']; agg['5xx'] += x['5xx']; agg.other += x.other;
}
...
export function createHistory() {
  ...
  function windowFor(fromMin, toMin) {
    if (!Number.isFinite(+fromMin) || !Number.isFinite(+toMin) || +toMin < +fromMin) return [];
    const b = BUCKET_MIN;
    let from = Math.floor(Math.floor(fromMin) / b) * b; // align to a bucket boundary
    const to = Math.floor(Math.floor(toMin) / b) * b;
    if ((to - from) / b > Math.floor(MAX_SPAN / b)) from = to - Math.floor(MAX_SPAN / b) * b; // clamp bar count
    const out = [];
    for (let start = from; start <= to; start += b) {
      const agg = zero();
      for (let m = start; m < start + b; m++) { const x = hist.get(m); if (x) addInto(agg, x); }
      out.push(withTotal(start, agg));
    }
    return out;
  }

  // The in-progress current bucket (for live incremental bar updates). Anchored
  // to the wall clock, not the last record, so the live edge keeps advancing
  // (with empty buckets) through a quiet patch or an upstream-feed outage —
  // the chart is time-based, so "now" should march on even with no data.
  function current() {
    const b = BUCKET_MIN;
    const now = nowMin();
    const start = Math.floor(now / b) * b;
    const agg = zero();
    for (let m = start; m <= now; m++) { const x = hist.get(m); if (x) addInto(agg, x); }
    return withTotal(start, agg);
  }

  // Earliest / latest minute we have data for (for the date picker bounds).
  // `latest` tracks the wall clock so the live edge advances even during a gap.
  function bounds() {
    let min = Infinity, max = -Infinity;
    for (const m of hist.keys()) { if (m < min) min = m; if (m > max) max = m; }
    const now = nowMin();
    return { earliest: Number.isFinite(min) ? min : now, latest: Math.max(now, Number.isFinite(max) ? max : now) };
  }

  // Total hits over the trailing `minutes` minutes, including the current one.
  function recentHits(minutes) {
    const end = nowMin();
    const start = end - (Math.max(1, minutes) - 1);
    let sum = 0;
    for (let m = start; m <= end; m++) {
      const x = hist.get(m);
      if (x) sum += x['2xx'] + x['3xx'] + x['4xx'] + x['5xx'] + x.other;
    }
    return sum;
  }

  return { record, windowFor, current, bounds, recentHits, MAX_SPAN, bucket: BUCKET_MIN };
}
