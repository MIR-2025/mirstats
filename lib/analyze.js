...
function buildPrompt({ label, hours, totals, reports, snap }) {
  ...
  return [
    `You are a web-server log analyst. Analyze the traffic for ${label} and give a concise, useful briefing: overall volume, notable spikes or anomalies, error-rate patterns (4xx/5xx), and security / attack activity. Be specific with numbers and hours. Call out anything that warrants attention. Keep it tight — short paragraphs or bullets, no preamble.`,
    ``,
...
export async function analyzeRange({ fromMin, toMin, label, logStream }) {
  ...
  const prompt = buildPrompt({ label: label || `${fromMin}–${toMin}`, hours: hourly(bars), totals, reports, snap });
  ...
  try {
    ...
    return { ok: true, analysis: text, model: MODEL, totals };
  } catch (e) {
