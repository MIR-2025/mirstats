...
export function createRoutes() {
  ...
  try {
    ...
    if (existsSync(SITES_FILE)) {
      ...
      sites = Array.isArray(raw)
        ? raw.filter((s) => s && s.source && s.url)
        : Object.entries(raw).map(([source, url]) => ({ source, url }));
    }
  ...
  if (sites.length) {
    console.log(`[routes] indexing ${sites.length} site(s); re-crawl every ${Math.round(RECRAWL_MS / 3600000)}h`);
    recrawl();
    const timer = setInterval(recrawl, RECRAWL_MS);
    timer.unref?.();
    return { isKnownRoute, status, recrawl, stop: () => clearInterval(timer) };
  }
  console.log('[routes] no sites in ' + SITES_FILE + ' — route indexing disabled');
  return { isKnownRoute: () => false, status: () => [], recrawl: async () => [], stop: () => {} };
}
